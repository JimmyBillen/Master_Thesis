...
from novak_tyson_ps import nullcline_and_boundary, calculate_mean_squared_error, compute_novak_dynamics_rk4, nullcline_x_ifo_x, nullcline_y_ifo_x, nullcline_x_ifo_y
from keras.models import load_model, Model
# from create_NN_FHN import normalization_with_mean_std, reverse_normalization
from mse_save_calc_2 import open_csv_and_return_all
from create_neural_network_NT import normalization_with_mean_std, reverse_normalization
import time
import math
...
def plot_lc_from_modelname(modelname, title_extra='', plot_bool=True, df=None):
    ...
    if df is None:
        ...
        absolute_path = os.path.dirname(__file__)
        relative_path = f"FHN_NN_loss_and_model_NT.csv"
    ...
    if plot_bool:
        # plot normal LC
        time, x_lc, y_lc = compute_novak_dynamics_rk4()
        plt.plot(x_lc, y_lc, 'r-', label=f'Trajectory')
        # Plot Nullcines
        # vdot
        # v = np.linspace(-2.5, 2.5, 1000)
        # plt.plot(v, nullcline_x_ifo_x(v), '--', color = "lime", label = r"$\dot{x}=0$ nullcline") #$w=v - (1/3)*v**3 + R * I$"+r" ,
        # # wdot
        # v = np.linspace(-2.5, 2.5, 1000)
        # plt.plot(v, nullcline_y_ifo_x(v), '--', color = "cyan", label = r"$\dot{y}=0$ nullcline") #$w=(v + A) / B$"+r" ,

        yvals = np.linspace(0, 4, 2000)
        plt.plot(nullcline_x_ifo_y(yvals), yvals, '--', color = "lime" ,label="xdot", linewidth=3, alpha=0.8)
        plt.plot(nullcline_y_ifo_x(yvals), yvals, '--', color = "cyan", label='ydot', linewidth=3, alpha=0.8)
...
if __name__ == '__main__':
    ...
    # cache_file = f'data_cache_{TAU}.pkl'
    ...
    [4,4]  (tot 9.33e-5(!!!)) accuracy (waarbij het de 5de beste nullcline error gebruikt)   
    '''
...
# =====================================  vb3: PCC effectivness testing by plotting on LC===========================================
    ...
    # search_modelname_of_point(option='option_3', learning_rate=0.01, max_epochs=499, nodes=2*[16], layers=2, total=40, normalization_methods=['min-max'], activation_functions=['relu', 'sigmoid'])
    # plot_lc_from_modelname('4d21d5d459ca44a7ac96eb321a805d25')
    # plot_lc_from_modelname('717918851c014a56b3cd72b6e3f7d3b8')
    # plot_lc_from_modelname('d8dea68ff1e14e3681162d519ed7ff51')
    # plot_lc_from_modelname('99881602c27742adbb0a1082c5831c6d')
    # plot_lc_from_modelname('c1313b62d12c4b04a7a0d6ca7beb392f')
    # plot_lc_from_modelname('e6d8473fe0bc4223859784dff902b1cb')
    # plot_lc_from_modelname('3eb7846bb0674d8f981e29a5dd6e4d27')
    # plot_lc_from_modelname('78209ce24a974e958007be8631e17aaf')
...
# search_modelname_of_point(option='option_3', learning_rate=0.01, max_epochs=499, nodes=[8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8], layers=16, total=360)
