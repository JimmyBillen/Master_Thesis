# Using a linear system (y=x) and training the neural network in the area 0-1 with 80% training data
...
history = model.fit(X_train, y_train, epochs=100, batch_size=32, validation_data=(X_val, y_val)) # in elke epoch 
...
plt.plot([0,0],[0,1], linestyle ='dashed', color='black')
plt.plot([0,1],[1,1], linestyle ='dashed', color='black')
plt.plot([0,1],[0,0], linestyle ='dashed', color='black')
plt.plot([1,1],[0,1], linestyle ='dashed', color='black')
plt.title('Validation and Prediction of chosen values as coordinates for y=x')
plt.xlabel('x')
plt.ylabel('y')
plt.legend()
