...
def plot_timeseries(plot=True, ax=None, TAU=7.5):
    # Plot the results
    time, v_values, w_values = compute_fitzhugh_nagumo_dynamics(TAU)

    v_values=v_values[int(len(v_values)/5):]
    w_values=w_values[int(len(w_values)/5):]
    time = time[int(len(time)/5):]

    if plot==True:
        fig, ax = plt.subplots(figsize=(10, 5))

    # ax.plot(time, v_values, label=r'$v$', color='C0')
    # ax.plot(time, w_values, label=r'$w$', color='C1')
    ax.scatter(time, v_values, color='C0', alpha=0.01, s=1)
    ax.scatter(time, w_values, color='C1', alpha=0.01, s=1)
    ax.plot([100], [200], label=r'$v$', color='C0')
    ax.plot([200], [300], label=r'$w$', color='C1')
    ...
    ax.set_xlabel('Time')
    if TAU == 1:
        ax.set_xlim(5.28, 34.32)
        ax.set_ylim(-0.535, 2.016)
        ax.set_ylabel('Amplitude')
    ax.set_xticks([])
    ax.set_yticks([])
    if TAU == 5:
        ax.set_xlim(8, 52)
        ax.set_ylim(-1.945, 2.06799)

    if TAU == 100:        
        ax.set_xlim(96, 624)
        ax.set_ylim(-2.19, 2.20)
        ax.legend(bbox_to_anchor=[1.01, 0], loc='lower left')
    # ax.grid()
    if plot==True:
...
if __name__ == '__main__':
    # derivative_plotter()
    plot_timeseries(TAU=1)
    plot_timeseries(TAU=7.5)
    plot_timeseries(TAU=100)
