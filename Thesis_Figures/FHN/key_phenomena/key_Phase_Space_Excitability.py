...
from key_Time_Series_Excitability import compute_fitzhugh_nagumo_dynamics, find_boundary_nullclines
...
def plot_limit_excite(u_nullcline=True, y_ifo_x=True, with_neural_network=False, mean_std=None, plot=True, ax=None):
    ...
    line = ax.plot(x_lc, y_lc, 'r-', label=f'Trajectory', linewidth=2, zorder=0, alpha=0)[0]

    add_arrow(line)

    ax.scatter(x_lc, y_lc, color='red', label=f'Trajectory', s=1, zorder=3, alpha=0.03)
