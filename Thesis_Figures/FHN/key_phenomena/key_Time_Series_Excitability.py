...
def plot_timeseries_excite(ax=None, plot=True):
    # Plot the results
    ...
    ax.scatter(time, w_values, color='C1', alpha=0.03, s=1)
    ax.scatter(time, v_values, color='C0', alpha=0.03, s=1)

    ax.plot([101, 102], [101, 102], label=r'$w$', color='C1')
    ax.plot([101, 102], [101, 102], label=r'$v$', color='C0')
    ...
    ax.legend(bbox_to_anchor=[0.26,0.64], loc='upper left', frameon=False)
    # ax.set_xticks([0,50])
    ax.set_xticks([])
    ax.set_yticks([])

    ax.set_xlim([-2.5, 52.5])
    ax.set_ylim([min(min(w_values), min(v_values))-0.2, max(max(w_values), max(v_values))+0.2])
