...
from time_series import compute_fitzhugh_nagumo_dynamics, find_boundary_nullclines
...
def limit_cycle(TAU):
    ...
    _, v_values, w_values = compute_fitzhugh_nagumo_dynamics(TAU)
    v_values=v_values[int(len(v_values)/5):]
    w_values=w_values[int(len(w_values)/5):]
...
def plot_limit_cycle(TAU=7.5, u_nullcline=True, y_ifo_x=True, with_neural_network=False, mean_std=None, plot=True, ax=None):
    ...
    x_lc, y_lc = limit_cycle(TAU)
    x_lc = x_lc[int(len(x_lc)/2):]
    y_lc = y_lc[int(len(y_lc)/2):]

    # ax.scatter(x_lc, y_lc, color='red', label=f'Limit cycle', zorder=5, alpha=0.3)
    # ax.plot(x_lc, y_lc, 'r-', label=f'Limit cycle', zorder=5)
    ax.scatter(x_lc, y_lc, color='red', zorder=10, alpha=0.01, s=2)
    ax.plot([100], [200], color='red', label=f'Limit cycle', zorder=10)
    ...
    ax.plot(v, nullcline_vdot(v), '--', color = "lime", label = r"$\dot{v}=0$", zorder=0)

        # wdot
    v = np.linspace(-2.5, 2.5, 1000)
    # plt.plot(v, nullcline_wdot(v), '--', color = "cyan", label = r"$w=(v + A) / B$"+r" ,$\dot{w}=0$ Nullcline")
    ax.plot(v, nullcline_wdot(v), '--', color = "cyan", label = r"$\dot{w}=0$", zorder=1)

    #     # Plotting a dot where the nullclines intersect
    dots = [0.409]
    dots_null = [(i + A) / B for i in dots]
    ax.scatter(dots, dots_null, color='black', marker='o', label='Fixed point',zorder=3)
