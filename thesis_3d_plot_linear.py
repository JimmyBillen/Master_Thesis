...
from FitzHugh_Nagumo_ps import nullcline_vdot, calculate_mean_squared_error, nullcline_wdot
from create_NN_FHN import calculate_derivatives
import numpy as np
from settings import TAU, R, I, A, B
...
# u_dot_t_data_again = u_dot_t_data.copy()
...
# Plot data points
# ax.scatter(v_t_data, u_dot_t_data, u_t_data, c='r', marker='o')
...
vdotmin, vdotplus = u_dot_t_data[min_index_neg], u_dot_t_data[min_index_pos]
...
if find_specific_v_value:
    min_index_pos_list = []
    min_index_neg_list = []
    # values_approx = np.linspace(min(v_t_data), max(v_t_data), 3)
...
plt.savefig(r"C:\Users\jimmy\OneDrive\Documents\Universiteit\KULeuven\Masterproef\Thesis_Fig\FHN\3d_phase_space_nullcline_linear.png")
