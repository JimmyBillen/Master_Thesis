...
from extra_functions import does_data_exist
import os
import pandas as pd
from ast import literal_eval
from keras.models import load_model, Model
# from novak_tyson_ps import nullcline_and_boundary, calculate_mean_squared_error, nullcline_vdot, nullcline_wdot, limit_cycle
...
from novak_tyson_ps import nullcline_and_boundary, calculate_mean_squared_error
import numpy as np
...
def open_csv_and_return_all(option, learning_rate, max_epochs, nodes, layers, amount, normalization_methods=None, activation_functions=None):
    ...
    # Open CSV file
    absolute_path = os.path.dirname(__file__)
    folder_path = os.path.join(absolute_path, f"VAL_vs_MSE_NT")
...
if __name__ == '__main__':
    # save_one_norm_two_act_MSE_vs_VAL(learning_rate=0.01, nodes=2*[16], layers=2, max_epochs=499, option='option_3', normalization_method=['min-max'], activation_functions=['relu'], amount_per_parameter=20, save=True)
    # save_one_norm_two_act_MSE_vs_VAL(learning_rate=0.01, nodes=2*[16], layers=2, max_epochs=499, option='option_3', normalization_method=['min-max'], activation_functions=['relu', 'sigmoid'], amount_per_parameter=20, save=True)
    ...
    plot_validation_vs_mse_one_norm_two_act(learning_rate=0.01, nodes=2*[16], layers=2, max_epochs=499, option='option_3', amount=20, normalization_methods=['min-max'], activation_functions=['relu', 'sigmoid'])
